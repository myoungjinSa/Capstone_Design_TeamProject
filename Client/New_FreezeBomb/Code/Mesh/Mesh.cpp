...
CHeightMapGridMesh::CHeightMapGridMesh(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList, int xStart, int zStart, int nWidth, int nLength, XMFLOAT3 xmf3Scale, XMFLOAT4 xmf4Color, void *pContext) : CMesh(pd3dDevice, pd3dCommandList)
{
	...
	//CalculateTriangleListTBNs(m_nVertices, m_pxmf3Positions, m_pxmf2TextureCoords0, m_pxmf3Tangents, m_pxmf3Bitangents, m_pxmf3Normals);
...
void CHeightMapGridMesh::CalculateTriangleListTBNs(int nVertices, XMFLOAT3 *pxmf3Positions, XMFLOAT2 *pxmf2TexCoords, XMFLOAT3 *pxmf3Tangents, XMFLOAT3 *pxmf3BiTangents, XMFLOAT3 *pxmf3Normals)
{
...
///////////////////////////////////////////////////////////////////////////////////////////
CCubeMeshDiffused::CCubeMeshDiffused(ID3D12Device *pd3dDevice,ID3D12GraphicsCommandList *pd3dCommandList,COLOR color,float fWidth,float fHeight,float fDepth)
	:CMesh(pd3dDevice,pd3dCommandList)
{
	m_nVertices = 36;
	m_d3dPrimitiveTopology = D3D_PRIMITIVE_TOPOLOGY_TRIANGLELIST;
	...
	{
		m_pxmf4Colors[i] = XMFLOAT4(red, green, blue, alpha);
	}

	m_pd3dColorsBuffer = ::CreateBufferResource(pd3dDevice, pd3dCommandList, m_pxmf4Colors, sizeof(XMFLOAT4) * m_nVertices, D3D12_HEAP_TYPE_DEFAULT, D3D12_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER, &m_pd3dColorsUploadBuffer);
	m_d3dColorsBufferView.BufferLocation = m_pd3dColorsBuffer->GetGPUVirtualAddress();
	m_d3dColorsBufferView.StrideInBytes = sizeof(XMFLOAT4);
	m_d3dColorsBufferView.SizeInBytes = sizeof(XMFLOAT4) * m_nVertices;

}

CCubeMeshDiffused::~CCubeMeshDiffused()
{
	if (m_pd3dColorsBuffer)
		m_pd3dColorsBuffer->Release();
	if (m_pxmf4Colors)
		delete[] m_pxmf4Colors;
}
void CCubeMeshDiffused::ReleaseUploadBuffers()
{
	CMesh::ReleaseUploadBuffers();

	if (m_pd3dColorsUploadBuffer)
	{
		m_pd3dColorsUploadBuffer->Release();
	}
	m_pd3dColorsUploadBuffer = NULL;
}
void CCubeMeshDiffused::OnPreRender(ID3D12GraphicsCommandList *pd3dCommandList, void *pContext)
{
	D3D12_VERTEX_BUFFER_VIEW pVertexBufferViews[2] = { m_d3dPositionBufferView, m_d3dColorsBufferView };
	pd3dCommandList->IASetVertexBuffers(m_nSlot, 2, pVertexBufferViews);
}
void CCubeMeshDiffused::Render(ID3D12GraphicsCommandList *pd3dCommandList, int nSubSet, int nInstance)
{
	UpdateShaderVariables(pd3dCommandList);
	OnPreRender(pd3dCommandList, NULL);
...
void CSkinnedMesh::UpdateShaderVariables(ID3D12GraphicsCommandList *pd3dCommandList)
{
	...
	if (m_pd3dcbBoneOffsets)
	{
		...
		pd3dCommandList->SetGraphicsRootConstantBufferView(4, d3dcbBoneOffsetsGpuVirtualAddress); 
	}
	if (m_pd3dcbBoneTransforms)
	{
		...
		pd3dCommandList->SetGraphicsRootConstantBufferView(5, d3dcbBoneTransformsGpuVirtualAddress); 
	}
}
