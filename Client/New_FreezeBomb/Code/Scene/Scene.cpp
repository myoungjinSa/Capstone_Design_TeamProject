			...
			for (auto enemyID : mapClients)
			{
				...
				if ((*iter).second->m_ppObjects[id]->GetBoundingBox().Intersects(m_pPlayer->GetBoundingBox()))
				{
					...
					if (m_pPlayer->GetIsBomb() == true && (*iter).second->m_ppObjects[id]->GetIsICE() == false)
					{
						...
						// 특수 동작중이면 충돌X
						int myStatus = m_pPlayer->m_pAnimationController->GetAnimationState();
						int status = (*iter).second->m_ppObjects[id]->m_pAnimationController->GetAnimationState();
						if (myStatus != CAnimationController::ANIMATIONTYPE::RAISEHAND
							&& myStatus != CAnimationController::ANIMATIONTYPE::USEGOLDHAMMER
							&& status != CAnimationController::ANIMATIONTYPE::RAISEHAND
							&& status != CAnimationController::ANIMATIONTYPE::USEGOLDHAMMER)
						{
