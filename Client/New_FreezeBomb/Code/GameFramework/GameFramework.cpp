...
void CGameFramework::ProcessPacket(char* packet)
{
	...
	case SC_ROUND_START:
	{
		...
		g_State = INGAME;
		m_pPlayer->SetIceCoolTime(0);
		m_pPlayer->SetIsShowCoolTime(false);
		m_RoundStartTime = chrono::steady_clock::now();
		m_RoundCountTime = chrono::steady_clock::now();
...
void CGameFramework::CheckIceChangeOk()
{
	...
	m_RoundCountTime = chrono::steady_clock::now();
	chrono::seconds currentSeconds = chrono::duration_cast<chrono::seconds> (m_RoundCountTime - m_RoundStartTime);
