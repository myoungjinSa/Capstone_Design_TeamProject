...
CZombieObject::CZombieObject(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList, ID3D12RootSignature *pd3dGraphicsRootSignature) : CGameObject()
{
	...
	CTexture *pZombieTexture= new CTexture(3, RESOURCE_TEXTURE2D, 0);
	...
	m_nMaterials = 1;
	m_ppMaterials = new CMaterial*[m_nMaterials];
	m_ppMaterials[0] = new CMaterial(3);
	m_ppMaterials[0]->SetShader(pZombieShader);
	m_ppMaterials[0]->SetReflection(1);

	m_ppMaterials[0]->SetTexture(pZombieTexture,0);
	//m_ppMaterials[0]->SetTexture(pZombieTexture[1], 1);
	//m_ppMaterials[0]->SetTexture(pZombieTexture[2], 2);
...
void CZombieObject::Render(ID3D12GraphicsCommandList *pd3dCommandList, CCamera *pCamera)
{
	...
	if (m_ppMaterials[0])
	{
		...
		{
			m_ppMaterials[0]->m_pShader->UpdateShaderVariable(pd3dCommandList, &m_xmf4x4World);
...
CMaterial::CMaterial(const int nTexture)
{
	m_nTextures = nTexture;
	m_ppTextures = new CTexture*[nTexture];
...
void CMaterial::SetTexture(CTexture *pTexture,int nTexture)
{
...
void CMaterial::SetTexture(ID3D12Resource *pResource, int nIndex)
{

	m_ppTextures[nIndex]->m_ppd3dTextures[nIndex]=pResource;
