...
ID3D12Resource *CreateTexture2DResource(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList, UINT nWidth, UINT nHeight, DXGI_FORMAT dxgiFormat, D3D12_RESOURCE_FLAGS d3dResourceFlags, D3D12_RESOURCE_STATES d3dResourceStates, D3D12_CLEAR_VALUE *pd3dClearValue)
{
	...
	D3D12_HEAP_PROPERTIES d3dHeapPropertiesDesc;
	::ZeroMemory(&d3dHeapPropertiesDesc, sizeof(D3D12_HEAP_PROPERTIES));
	d3dHeapPropertiesDesc.Type = D3D12_HEAP_TYPE_DEFAULT;			//디폴트 힙: gpu만 읽고 쓸수 있는 힙
	d3dHeapPropertiesDesc.CPUPageProperty = D3D12_CPU_PAGE_PROPERTY_UNKNOWN;
	d3dHeapPropertiesDesc.MemoryPoolPreference = D3D12_MEMORY_POOL_UNKNOWN;
	d3dHeapPropertiesDesc.CreationNodeMask = 1;
	d3dHeapPropertiesDesc.VisibleNodeMask = 1;
	...
	d3dTextureResourceDesc.Flags = d3dResourceFlags;		//GenericRead상태 
...
ID3D12Resource *CreateTextureResourceFromFile(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList, wchar_t *pszFileName, ID3D12Resource **ppd3dUploadBuffer, D3D12_RESOURCE_STATES d3dResourceStates)
{
	//DDS 텍스쳐 리소스 생성
	ID3D12Resource *pd3dTexture = NULL;
	std::unique_ptr<uint8_t[]> ddsData;	//uint8_t --> 0~255 까지 표현 가능한 타입의 unique_ptr(스마트 포인터)	
										//unique_ptr 은 리소스를 고유하게 관리한다. 
										//unique_ptr 를 사용하면 동적으로 힙 영역에 할당된 인스턴스에 대한 소유권을 얻고 
										//unique_ptr 객체가 사라지면 해당 인스턴스를 해제하게 된다. 
										// 소유권을 갖기 때문에 인스턴스를 소유하는 unique_ptr은 동시에 두개가 되어선 안된다. 
										// 또한 이동은 가능하지만 복사는 안된다.

	//서브리소스 : 텍스처 리소스의 부분 집합(텍스쳐는 서브리소스들의 집합(배열)으로 취급)
	//			  각 서브리소스는 인덱스로 접근할 수 있음
	//			  버퍼는 하나의 서브리소스로 구성됨()
	//			  텍스쳐는 텍스쳐의 유형에 따라 서브리소스의 개수가 달라진다. 텍스쳐 배열의 크기와 밉맵 레벨에 따라 달라진다.
	...
	//리소스와 내부적인 힙을 동시에 생성 , 업로드힙 생성 
	// 업로드힙의 초기 상태 -> D3D12_RESOURCE_GENERIC_READ : ONLY READ상태
	pd3dDevice->CreateCommittedResource(&d3dHeapPropertiesDesc, D3D12_HEAP_FLAG_NONE, &d3dResourceDesc, D3D12_RESOURCE_STATE_GENERIC_READ, NULL, __uuidof(ID3D12Resource), (void **)ppd3dUploadBuffer);
	...
	// 픽셀 데이터를 업로드 힙으로 복사하고 업로드 힙을 텍스쳐 리소스로 복사한다.
	...
	d3dResourceBarrier.Transition.StateBefore = D3D12_RESOURCE_STATE_COPY_DEST;		//복사 연산의 목표로 사용될때
	d3dResourceBarrier.Transition.StateAfter = d3dResourceStates;					//D3D12_RESOURCE_STATE_GENERIC_READ : 업로드 힙의 초기 상태
	d3dResourceBarrier.Transition.Subresource = D3D12_RESOURCE_BARRIER_ALL_SUBRESOURCES;
	pd3dCommandList->ResourceBarrier(1, &d3dResourceBarrier);
