...
void CStandardMesh::LoadMeshFromFile(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList, FILE *pInFile)
{
	...
	UINT nReads = (UINT)::fread(&m_nVertices, sizeof(int), 1, pInFile);		//정점개수 
	...
	for ( ; ; )
	{
		...
		{
			nReads = (UINT)::fread(&(m_nSubMeshes), sizeof(int), 1, pInFile);//서브메쉬 개수 읽는다
			if (m_nSubMeshes > 0)
			{
				m_pnSubSetIndices = new int[m_nSubMeshes]; //서브 메쉬 1개를 가리키는 인덱스 배열
				m_ppnSubSetIndices = new UINT*[m_nSubMeshes]; //서브메쉬 값을 가리키는 정수 배열
...
void CSkinnedMesh::LoadSkinInfoFromFile(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList, FILE *pInFile)
{
	...
	for ( ; ; )
	{
		...
		if (!strcmp(pstrToken, "<BonesPerVertex>:"))	//하나의 뼈의 움직임에 영향을 받는 정점의 개수
		{
		...
		{
			nReads = (UINT)::fread(&m_nSkinningBones, sizeof(int), 1, pInFile);	//스키닝 프레임 개수
			if (m_nSkinningBones > 0)
			{
				m_ppstrSkinningBoneNames = new char[m_nSkinningBones][64];	//뼈 이름
				m_ppSkinningBoneFrameCaches = new CGameObject*[m_nSkinningBones]; //뼈 개수 만큼 GameObject생성
				for (int i = 0; i < m_nSkinningBones; i++)
				{
					nReads = (UINT)::fread(&nStrLength, sizeof(BYTE), 1, pInFile);
					nReads = (UINT)::fread(m_ppstrSkinningBoneNames[i], sizeof(char), nStrLength, pInFile); //뼈 개수 이름
