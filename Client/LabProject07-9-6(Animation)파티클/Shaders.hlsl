...
//	uint					gnTimeElapsed : packoffset(c9);
};

cbuffer cbParticleInfo : register(b5)
{
	uint					gnTimeElapsed : packoffset(c0);
}
...
Texture2D gtxtIceParticle : register(t17);
...
{
	float4 cColor = gtxtIceParticle.Sample(gssWrap, input.uv);
...
Texture2D gtxtParticle : register(t14);
