...
CGameObject *CGameObject::LoadGeometryFromFile(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList, ID3D12RootSignature *pd3dGraphicsRootSignature, char *pstrFileName, CShader *pShader)
{
	...
	CGameObject *pGameObject = CGameObject::LoadFrameHierarchyFromFile(pd3dDevice, pd3dCommandList, pd3dGraphicsRootSignature, NULL, pInFile, pShader, 0);
...
CSkyBox::CSkyBox(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList, ID3D12RootSignature *pd3dGraphicsRootSignature) : CGameObject(1)
{
	...
	pSkyBoxTexture->LoadTextureFromFile(pd3dDevice, pd3dCommandList, L"SkyBox/SkyBox_3.dds", 0);
...
void CSuperCobraObject::Animate(float fTimeElapsed,CCamera *pCamera)
{
...
void CGunshipObject::Animate(float fTimeElapsed,CCamera *pCamera)
{
	if (m_pMainRotorFrame)
	{
...
void CMi24Object::Animate(float fTimeElapsed,CCamera *pCamera)
{
	if (m_pMainRotorFrame)
	{
		XMMATRIX xmmtxRotate = XMMatrixRotationY(XMConvertToRadians(360.0f * 2.0f) * fTimeElapsed);
...
CDeadTreeObject::CDeadTreeObject(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList, ID3D12RootSignature *pd3dGraphicsRootSignature)
{

}

CDeadTreeObject::~CDeadTreeObject() {

}


CPineTreeObject::CPineTreeObject(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList, ID3D12RootSignature *pd3dGraphicsRootSignature) 
{

}

CPineTreeObject::~CPineTreeObject() {

}

CDeerObject::CDeerObject(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList, ID3D12RootSignature *pd3dGraphicsRootSignature)
{

}

CDeerObject::~CDeerObject() {
	...
	:CBillboardObject()
{
	...
	pParticleTexture->LoadTextureFromFile(pd3dDevice, pd3dCommandList, L"Model/Textures/fire.dds", 0);

	CBillboardShader *pParticleShader = new CBillboardShader();
	...
	CScene::CreateShaderResourceViews(pd3dDevice, pParticleTexture, 15, false);
...
CParticleBillboardObject::~CParticleBillboardObject()
{

}

void CParticleBillboardObject::Animate(float fTimeElapsed,CCamera *pCamera)
{
	CBillboardObject::Animate(fTimeElapsed,pCamera);


	static float totalTime = 0.0f;
	totalTime += 5.0f*fTimeElapsed;
	
	if (totalTime > 1.0f) {
		m_fParticleTime += (UINT)totalTime;
		totalTime = 0.0f;
	}
...
void CParticleBillboardObject::Render(ID3D12GraphicsCommandList *pd3dCommandList, CCamera *pCamera)
{
	CGameObject::Render(pd3dCommandList, pCamera);	
...
void CParticleBillboardObject::CreateShaderVariables(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList)
{
	UINT ncbElementBytes = ((sizeof(CB_PARTICLE_INFO) + 255) &~255);

	m_pd3dcbParticleObjects = ::CreateBufferResource(pd3dDevice, pd3dCommandList, NULL, ncbElementBytes, D3D12_HEAP_TYPE_UPLOAD, D3D12_RESOURCE_STATE_VERTEX_AND_CONSTANT_BUFFER, NULL);

	m_pd3dcbParticleObjects->Map(0, NULL, (void**)&m_pcbMappedParticlObjects);
}

#define _WITH_DEBUG_DATA

void CParticleBillboardObject::ReleaseShaderVariables()
{
	if (m_pd3dcbParticleObjects)
	{
		m_pd3dcbParticleObjects->Unmap(0, NULL);
		m_pd3dcbParticleObjects->Release();
	}
}

void CParticleBillboardObject::UpdateShaderVariable(ID3D12GraphicsCommandList *pd3dCommandList, XMFLOAT4X4 *pxmf4x4World)
{
	XMFLOAT4X4 xmf4x4World;
	XMStoreFloat4x4(&xmf4x4World, XMMatrixTranspose(XMLoadFloat4x4(pxmf4x4World)));
	pd3dCommandList->SetGraphicsRoot32BitConstants(1, 16, &xmf4x4World, 0);

	D3D12_GPU_VIRTUAL_ADDRESS d3dParticleGPUVirtualAddress = m_pd3dcbParticleObjects->GetGPUVirtualAddress();
